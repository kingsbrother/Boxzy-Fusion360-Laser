/**
  ...
  ######################################################
  Rev01, 06/30/2019 B.Glaser
  Modified Fusion360 Posting Configuration for Boxzy Laser
    - All changes will be tagged with Rev number and most with "Boxzy"
  ######################################################
*/

description = "Smoothie Boxzy Laser Rev01";
vendor = "Smoothie";
...
capabilities = CAPABILITY_JET; // Rev01 Boxzy, removed "CAPABILITY_MILLING | "
...
allowedCircularPlanes = 0; // (REV01 Boxzy) DO NOT allow any circular motion
...
properties = {
  ...
  laserEtchPower: 15, // the laser etching power (Rev01 Boxzy, changed from .1 to 15)
  laserPower: 100, // laser cutting power (Rev01 Boxzy, changed from 1 to 100)
  useLaserM3M5: false, // activate/deactivate laser using M3/M5 (Rev01 Boxzy, changed from true to false)
...
var coolants = {
  ...
  throughTool: {on: 106},
  air: {on: 106},
  airThroughTool: {on: 106},
  suction: {on: 106},
  floodMist: {on: 106},
  floodThroughTool: {on: 106},
  off: 107
};
...
var powerFormat = createFormat({decimals:0}); // Rev01 Boxzy, Changed from 2 to 0 for Boxzy
...
var powerOutput = createVariable({prefix:"L", force:false}, powerFormat); // (Rev01 Boxzy, Changed "S" to "L" for Boxzy)
...
function onLinear(_x, _y, _z, feed) {
  ...
  //var l = powerOutput.format(getPower()); // Rev 01 Added for Boxzy, but didn't work.  Need to manually enter "L"
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      writeBlock(gMotionModal.format(1), x, y, z, f, "L"+getPower());  // Rev01 Boxzy:  This is the big change
